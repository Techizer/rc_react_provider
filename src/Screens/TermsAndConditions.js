...
import { Colors,  Font, mobileH, mobileW, config, LanguageConfiguration } from '../Provider/utilslib/Utils'
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#fff' }}>
        <ScreenHeader
          ...
          title={(() => {
            if (this.state.pagename == 0) return LanguageConfiguration.AboutRootscare[config.language]
            if (this.state.pagename == 2) return LanguageConfiguration.TermsandConditions[config.language]
            if (this.state.pagename == 1) return LanguageConfiguration.PrivacyPolicy[config.language]
          })()}
